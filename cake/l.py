...
class LCD:
    ...
    def __init__(self, cols=20, rows=4, dotsize=8, charmap='A02', auto_linebreaks=True):
        ...
        """
        assert dotsize in [
            8, 10], 'The ``dotsize`` argument should be either 8 or 10.'
        ...
        if charmap == 'A00':
            ...
            raise ValueError(
                'The ``charmap`` argument must be either ``A00`` or ``A02``')
        ...
        if self.data_bus_mode == c.LCD_4BITMODE:
            ...
            raise ValueError(
                'Invalid data bus mode: {}'.format(self.data_bus_mode))
        ...
        self.command(c.LCD_DISPLAYCONTROL |
                     self._display_mode | self._cursor_mode)
        c.usleep(50)
        ...
        self.command(c.LCD_ENTRYMODESET | self._text_align_mode |
    ...
    def _set_text_align_mode(self, value):
        if value == 'left':
            ...
            raise ValueError(
                'Text align mode must be either `left` or `right`')
        self.command(c.LCD_ENTRYMODESET | self._text_align_mode |
                     self._display_shift_mode)
        c.usleep(50)
    ...
    def _set_write_shift_mode(self, value):
        if value == 'cursor':
            ...
            raise ValueError(
                'Write shift mode must be either `cursor` or `display`.')
        self.command(c.LCD_ENTRYMODESET | self._text_align_mode |
                     self._display_shift_mode)
        c.usleep(50)
    ...
    def _set_cursor_mode(self, value):
        if value == 'hide':
            ...
            raise ValueError(
                'Cursor mode must be one of `hide`, `line` or `blink`.')
        self.command(c.LCD_DISPLAYCONTROL |
                     self._display_mode | self._cursor_mode)
        c.usleep(50)
